...
void bind_base_math(py::module_ &m) {
  ...
  py::class_<Isis::Calculator>(m, "Calculator")
      ...
      .def("push",
           [](Isis::Calculator &self, const std::vector<double> &vec) {
             QVector<double> qvec = stdVectorToQVector(vec);
             self.Push(qvec);
           },
           py::arg("vector"),
           "Push a vector of values onto the calculator stack")
      .def("pop",
           [](Isis::Calculator &self, bool keep_specials) {
             return qVectorToStdVector(self.Pop(keep_specials));
           },
           py::arg("keep_specials") = false,
           "Pop a vector from the calculator stack")
  ...
  py::class_<Isis::Affine>(m, "Affine")
      ...
      .def("solve",
           [](Isis::Affine &self, const std::vector<double> &x, const std::vector<double> &y,
              const std::vector<double> &xp, const std::vector<double> &yp, int n) {
             self.Solve(x.data(), y.data(), xp.data(), yp.data(), n);
           },
           py::arg("x"), py::arg("y"), py::arg("xp"), py::arg("yp"), py::arg("n"),
           "Solve for affine transformation coefficients")
      ...
      // Coefficient access
      .def("coefficients",
           [](const Isis::Affine &self, int var) {
             return self.Coefficients(var);
           },
           py::arg("variable"),
           "Get forward transformation coefficients")
      .def("inverse_coefficients",
           [](const Isis::Affine &self, int var) {
             return self.InverseCoefficients(var);
           },
           py::arg("variable"),
           "Get inverse transformation coefficients")
  ...
  py::class_<Isis::BasisFunction>(m, "BasisFunction")
      ...
      .def("set_coefficients",
           [](Isis::BasisFunction &self, const std::vector<double> &coefs) {
             self.SetCoefficients(coefs);
           },
           py::arg("coefficients"),
           "Set the function coefficients")
      .def("evaluate",
           py::overload_cast<const std::vector<double> &>(&Isis::BasisFunction::Evaluate),
           py::arg("variables"),
           "Evaluate the function with multiple variables")
      .def("evaluate",
           py::overload_cast<const double &>(&Isis::BasisFunction::Evaluate),
           py::arg("variable"),
           "Evaluate the function with a single variable")
      .def("expand",
           &Isis::BasisFunction::Expand,
           py::arg("variables"),
           "Expand the basis function")
