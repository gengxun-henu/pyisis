...
public:
  ...
  inline double radius(const double dx, const double dy, const double dz) const
  {
    ...
    if (qFuzzyCompare(v1max + 1.0, 1.0))
    {
      return (0.0);
    }
  ...
  for (int brick = 1; brick <= tile.Bricks(); brick++)
  {
    ...
    for (int index = 0; index < npixels; index++)
    {
          ...
          (tproj->SetWorld(samp, line)))
      {
        ...
        {
          if (tproj->Latitude() < tproj->MinimumLatitude())
            mapit = false;
          if (tproj->Latitude() > tproj->MaximumLatitude())
            mapit = false;
          if (tproj->Longitude() < tproj->MinimumLongitude())
            mapit = false;
          if (tproj->Longitude() > tproj->MaximumLongitude())
            mapit = false;
        }
        ...
        if (mapit)
        {
          ...
          { // ( neighbor_search == search_type)
            results = cloud_t.neighbor_query(cpt, neighbors);
          }

          // Extract points and prepare for processing
          MapPointCollector mpoint;
          if (ResultType::Radius == results.type())
            mpoint.setSearchType(MapPointCollector::Radius);
          else
            mpoint.setSearchType(MapPointCollector::NearestNeighbor);

          // Extract point set and optionally apply noise filter
          results.forEachPair(mpoint);
          if (do_radius_filter)
          {
            mpoint.removeNoise(sigma);
          }
