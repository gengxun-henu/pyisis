...
| Isolate Windows package caches by ISIS release | Run `32440542931` failed during ISIS 10 setup with `InvalidArchiveError` on locked cache DLLs. Version-specific caches prevent cross-environment extraction collisions, and `CONDA_ALWAYS_COPY=true` prevents the persistent environments from hard-linking DLLs back into those caches. |
| Serialize only Windows Conda package extraction | Run `32441175716` reproduced `InvalidArchiveError` in a fresh isolated cache, including a missing long Viskores header. Conda 26 derives its hard-coded extraction pool (maximum three) from `os.cpu_count()`; Python 3.14 honors `PYTHON_CPU_COUNT=1`. Applying it only to setup-miniconda preserves libmamba and build parallelism. |
| Use a short Windows Conda root | `LongPathsEnabled=1` was already set, but Conda package handling still failed on a 263-character Viskores extraction path. `D:\pyisis-conda` preserves D: storage and version isolation while removing enough prefix length for the archive. |
| Resolve self-hosted dependency prefixes explicitly | Setup-miniconda successfully updates the path-based environment but, with shell initialization and auto-activation disabled, global `python` remains `C:\Python312`. The explicit prefix is already the workflow contract and is now checked for `python.exe` before exporting PATH. |
| Make Windows runtime staging importable by file path | The PR gate invokes the unit file directly, so `sys.path` contains `tests/unitTest` rather than the repository root. The staging script now retains the package import when available and otherwise loads its known sibling with `importlib`, without global path mutation. |
| Index Windows DLLs case-insensitively on Linux | `Path.glob("*.dll")` follows host filesystem case rules, but PE imports and Windows filenames are case-insensitive. Filtering enumerated file suffixes with `.lower()` makes Linux-hosted unit evidence match production Windows behavior. |
| Give Windows binding builds a Ninja-specific cache | MSVC activation succeeded and `cl.exe` was visible, but scikit-build reused `build/{wheel_tag}` configured by Visual Studio with an unknown compiler. `build/{wheel_tag}-ninja` avoids the poisoned cache and makes the requested parallelism explicit. |
