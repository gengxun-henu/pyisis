...
Detector-free methods represent an alternative paradigm. LoFTR [6] employs self-attention and cross-attention transformer layers to establish dense correspondences without explicit feature detection, showing particular strength on textureless scenes. EfficientLoFTR [15] further improves computational efficiency through sparse attention mechanisms.

In the planetary domain, recent work has evaluated deep learning matchers for specific applications. Geo-LoFTR [16] incorporates geometric context from digital terrain models for Mars rotorcraft localization. Feature matching evaluations on multisource planetary remote sensing imagery [17] have demonstrated that deep learning methods can outperform classical approaches under challenging conditions, but no single method dominates across all scenarios. Deep learning-based feature extraction for Chang'e-4 Yutu-2 lunar rover imagery [18] has shown improved surface reconstruction quality.
...
In remote sensing, adaptive matching networks [22] have been proposed for multimodal satellite image registration, and weak-texture matching methods [23] address the challenge of sparse features in certain terrain types. However, no prior work has developed an adaptive routing system specifically designed for planetary photogrammetry that leverages mission-specific metadata (SPICE kernels, sensor models) for illumination-aware matching strategy selection.
...
**SIFT with FLANN Matching**: Uses the same SIFT detector but replaces brute-force search with FLANN (Fast Library for Approximate Nearest Neighbors) [25] using randomized kd-trees (4 trees, 32 checks). FLANN provides substantial speedup for large descriptor sets with minimal accuracy loss.
...
**Table V** summarizes the routing stability under threshold perturbation:
...
The control network construction pipeline integrates PyISIS geometry, the adaptive matching system, and coordinate transformation to produce ISIS-compatible control network files (`.net`). The workflow proceeds as follows:
...
**Table I** summarizes the per-pair matching results:
...
**Table II** presents the distribution of routing decisions across the six pairs:
...
**Table III** compares the computational cost of different matching stages:
...
**Table IV** compares the adaptive pipeline against single-method baselines:
...
[15] Q. Zhu et al., "Efficient LoFTR: Semi-Dense Local Feature Matching with Sparse-Like Speed," in *Proc. IEEE/CVF Conference on Computer Vision and Pattern Recognition (CVPR)*, 2024.

[16] "Geometry-aided Vision-based Localization of Future Mars Rotorcraft in Challenging Illumination Conditions," arXiv:2502.09795, 2025.

[17] "Comparison and Evaluation of Feature Matching Methods for Multisource Planetary Remote Sensing Imagery," *The Photogrammetric Record*, vol. 39, no. 188, Oct. 2024.
...
[25] M. Muja and D. G. Lowe, "Fast Approximate Nearest Neighbors with Automatic Algorithm Configuration," in *Proc. International Conference on Computer Vision Theory and Applications (VISAPP)*, 2009, pp. 331–340.
...
[27] V. Balntas, K. Lenc, A. Vedaldi, and K. Mikolajczyk, "HPatches: A Benchmark and Evaluation of Handcrafted and Learned Local Descriptors," in *Proc. IEEE/CVF Conference on Computer Vision and Pattern Recognition (CVPR)*, 2017, pp. 3852–3861.
