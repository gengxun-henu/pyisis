...
template <class T, class D> class PointCloudTree {
    ...
    public:
      ...
      #if NANOFLANN_VERSION >= 0x160
        std::vector<nanoflann::ResultItem<unsigned int, double> > new_matches;
        int n = m_kd_index.radiusSearch(point.array(), radius_sq, new_matches,
                NanoflannSearchParams());
        matches.reserve(new_matches.size());
        for (size_t i = 0; i < new_matches.size(); i++) {
          matches.push_back(std::make_pair(static_cast<size_t>(new_matches[i].first),
                   new_matches[i].second));
        }
      #else
      ...
      PointCloudSearchResult<T,D> neighbor_query(const T &point,
        ...
        for (int i = 0; i < neighbors; i++) {
          indices[i] = static_cast<size_t>(kd_indices[i]);
        }
