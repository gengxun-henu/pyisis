...
| Linux x86_64 | The pinned ISIS 9.0.0 conda environment under `ports/linux/env/`, the pinned GCC 12 toolchain, and the PyPA `manylinux_2_28_x86_64` container | A self-contained `usgs-pyisis` wheel plus `usgs-pyisis-isisdata-minimal` in the GitHub Release wheelhouse | CI builds CPython 3.12 `manylinux_2_35_x86_64` wheels, keeps a stricter GLIBC 2.28 symbol gate, enforces the auditwheel 2.35 policy, and clean-installs the same wheelhouse on Ubuntu 22.04 and 24.04 |
...
22.04/24.04 clean-install validation. The validated wheelhouses may be called
GitHub Release-ready, but they are not published to PyPI by default. Do not
claim universal cross-platform support until additional platforms and Python
ABI combinations are built and tested.
