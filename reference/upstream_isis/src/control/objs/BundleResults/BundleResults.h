  ...
  class BundleResults : public QObject {
    ...
    public:
      ...
      BundleResults &operator=(const BundleResults &src);
                  void copyForPyBind(const BundleResults &src) {
                        if (&src == this) {
                              return;
                        }
                        ...
                        if (src.m_correlationMatrix) {
                              m_correlationMatrix->setCovarianceFileName(src.m_correlationMatrix->covarianceFileName());
                              m_correlationMatrix->setCorrelationFileName(src.m_correlationMatrix->correlationFileName());

                              QMap<QString, QStringList> imagesAndParameters;
                              if (src.m_correlationMatrix->imagesAndParameters()) {
                                    imagesAndParameters = *src.m_correlationMatrix->imagesAndParameters();
                              }
                              m_correlationMatrix->setImagesAndParameters(imagesAndParameters);
                        }

                        m_cumPro = new StatCumProbDistDynCalc();
                        if (src.m_cumPro) {
                              *m_cumPro = *src.m_cumPro;
                        }

                        m_cumProRes = new StatCumProbDistDynCalc();
                        if (src.m_cumProRes) {
                              *m_cumProRes = *src.m_cumProRes;
                        }
                  }
      void initialize();
