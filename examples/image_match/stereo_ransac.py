...
def compute_ransac_retained_mask(
    ...
    *,
    ...
    This is a lower-level helper than ``filter_stereo_pair_keypoints_with_ransac``:
    it takes raw numpy point arrays and returns only the retained mask, without
    building KeypointFile objects.  Used by measure-level DOM RANSAC workflows
    that need to know which original indices were dropped.
    """
    normalized_mode = _normalize_ransac_mode(ransac_mode)
    normalized_model = _normalize_ransac_model(ransac_model)
    threshold = float(ransac_reproj_threshold)
