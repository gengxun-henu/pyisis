...
- `csv2table`、`ocams2isis` 与 `eisstitch` 在 ISIS 9/10 和 Linux/Windows
  均采用原生 ISIS APP 执行边界，不公开 `UserInterface`，也不新增 Python
  wrapper、helper 或进程内绑定。Python 编排如有需要可使用标准库
  `subprocess`。
- `csv2table` 的公开调用方式为：Linux 使用
