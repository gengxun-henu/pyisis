...
| Linux x86_64 | The pinned ISIS 9.0.0 conda environment under `ports/linux/env/`, the conda C++ compiler, and the PyPA `manylinux_2_28_x86_64` container | `usgs-pyisis`, `usgs-pyisis-runtime-linux-x86_64`, and `usgs-pyisis-isisdata-minimal` wheels | CI is configured to build CPython 3.12 wheels in the manylinux 2.28 container, enforce GLIBC/auditwheel gates, and clean-install the same wheelhouse on Ubuntu 22.04 and 24.04; PyPI release remains blocked until that workflow passes |
...
Linux support as a configured CPython 3.12 manylinux 2.28 build with Ubuntu
