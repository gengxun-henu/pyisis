...
namespace Isis {
  ...
  class BundleSolutionInfo : public QObject {
    ...
    public:
      ...
      void setOutputStatisticsForPyBind(const BundleResults &statisticsResults) {
        if (!m_statisticsResults) {
          m_statisticsResults = new BundleResults();
        }
        m_statisticsResults->copyForPyBind(statisticsResults);
      }
      ...
      QString id() const;
      QString inputControlNetFileNameForPyBind() const {
        if (!m_inputControlNetFileName) {
          return "";
        }
      ...
      QString inputLidarDataFileNameForPyBind() const {
      ...
      BundleResults *cloneBundleResultsForPyBind() const {
        if (!m_statisticsResults) {
          return NULL;
        }

        BundleResults *results = new BundleResults();
        results->copyForPyBind(*m_statisticsResults);
        return results;
      }
